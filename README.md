...
“亚马逊竞品情报系统”是一个个人SaaS工具。它通过**自动化Web爬虫**，7x24小时抓取、存储和分析竞品数据。系统将通过一个**现代Web界面**（由AI辅助生成）展示历史趋势和变化对比，并通过**纯文本电子邮件**发送**主动警报**。
...
| ✅ **纯文本电子邮件**主动警报 | ❌ **后端负责HTML Diff高亮** (移至前端实现) |
...
| **P0** | 我 (运营) | 当一个竞品**价格变动**或**库存低于阈值**时 | **立即**在我的邮箱里收到**纯文本通知**。 |
| **P0** | 我 (运营) | 当竞品**标题或五点**发生变化时 | 在邮件中收到**变更通知**（前端负责高亮对比）。 |
| **P1** | 我 (运营) | 当一个竞品**获得了新的1-3星差评**时 | **立即**在邮件中收到通知，并查看差评内容，以便分析他们的弱点。 |
...
* **F-DATA-007 (推送器 - 电子邮件):**
    * 所有被触发的警报，必须被格式化为 **纯文本电子邮件**。
    * **收件人：** 从配置中读取 (e.g., `alert.email.to=13337996401@163.com`)。
...
| | **NFR-S-002:** 所有敏感密钥（数据库密码、代理APIKey、**Spring Mail(SMTP)密码**）**绝不能**硬编码在代码中，必须使用环境变量或 `.properties` 文件注入。 |
...
| **配置** | **NFR-C-001:** 必须在 `application.yml` 中提供 **Spring Mail** (SMTP主机/端口/用户名/密码) 和**警报收件人** (`alert.email.to`) 的配置项。 |
...
* **纯文本电子邮件警报**（价格、差评、库存、**五点、标题**）推送功能稳定。
