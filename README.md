...
5. **提取数据**：
    *   若捕获到“仅剩 X 件”提示，则记录精确库存 X。
    *   **策略更新**：若成功修改为 999 且**无任何库存不足/限购提示**，系统默认判定库存充足，记录为 **999**。
...
#### 3. 优惠券与折扣抓取 (V2.1 优化)
系统优化了对 Coupon 和 Deal 信息的解析逻辑：
*   **精准折扣**：优先抓取精确的折扣百分比（如 "-29%"），而非通用的 "Up to x% off" 文本。
*   **多源解析**：同时检测 Coupon 徽章、秒杀 (Lightning Deal) 标识以及价格下方的 "Save: $X (Y%)" 信息。
