-- Spyglass 数据库建表脚本（PostgreSQL）
-- 该文件包含用于创建主要表的 SQL：asin, asin_history, scrape_task
-- 注意：如果在生产使用，请把此脚本作为迁移（Flyway / Liquibase / custom）的一部分。
...
-- 可选：示例数据（取消注释以插入）
-- INSERT INTO asin(asin, site, nickname, inventory_threshold) VALUES ('B000EXAMPLE', 'US', '示例', 10);
